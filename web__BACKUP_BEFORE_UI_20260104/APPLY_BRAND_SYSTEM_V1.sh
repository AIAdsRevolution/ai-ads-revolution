...
echo "== 1) globals.css -> Brand System V1 =="
cat > src/app/globals.css <<'EOF'
@tailwind base;
@tailwind components;
@tailwind utilities;

/* AI Ads Revolution — Brand System V1 */
:root{
  --bg0:#070A12;
  --bg1:#0B1020;
...
html,body{
  background:
    radial-gradient(1200px 800px at 20% 0%, rgba(79,70,229,.18), transparent 55%),
...
@layer components{
  ...
  .ui-h1{ @apply text-4xl md:text-6xl font-semibold tracking-tight; }
  .ui-h2{ @apply text-2xl md:text-4xl font-semibold tracking-tight; }
  .ui-sub{ @apply mt-6 text-lg text-slate-300 max-w-2xl; }
  ...
  .ui-btn-primary{
    background: linear-gradient(135deg, var(--ai), rgba(79,70,229,.72));
    color: var(--text);
    box-shadow: 0 0 0 1px rgba(79,70,229,.35), 0 18px 40px rgba(79,70,229,.22);
  }
  ...
  .ui-hero-glow{
    ...
    inset:-35%;
    background:
      radial-gradient(circle at 30% 20%, rgba(79,70,229,.35), transparent 40%),
      radial-gradient(circle at 80% 35%, rgba(34,211,238,.22), transparent 45%);
    filter: blur(18px);
    pointer-events:none;
  }
}
...
echo "== 2) Home V4 components (create/overwrite) =="
rm -rf components/home-v4
...
export default function Hero() {
  return (
    <header className="relative overflow-hidden border-b border-white/5">
      ...
      <div className="ui-container pt-16 md:pt-24 pb-16 md:pb-24 relative z-10">
        <div className="flex flex-wrap gap-2">
          <span className="ui-chip"><span className="h-2 w-2 rounded-full" style={{background:"var(--ok)"}} /> Live</span>
          <span className="ui-chip"><span className="h-2 w-2 rounded-full" style={{background:"var(--glow)"}} /> Realtime signals</span>
...
echo "== 3) Update page.tsx to use Home V4 =="
