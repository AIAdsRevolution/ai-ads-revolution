...
echo "== Stop dev servers =="
PIDS="$(lsof -ti :3000 2>/dev/null || true)"; [ -n "$PIDS" ] && kill -9 $PIDS || true
PIDS="$(lsof -ti :3001 2>/dev/null || true)"; [ -n "$PIDS" ] && kill -9 $PIDS || true

echo "== Ensure correct folder (ROOT/components) =="
mkdir -p components/home-v2

echo "== Write FeatureGrid.tsx in ROOT/components/home-v2 =="
cat > components/home-v2/FeatureGrid.tsx <<'EOF'
...
echo "== PROVA: file exists? =="
ls -la components/home-v2 | sed -n '1,120p'
test -f components/home-v2/FeatureGrid.tsx && echo "✅ FeatureGrid.tsx PRESENTE" || (echo "❌ FeatureGrid.tsx NON PRESENTE" && exit 1)
